...
interface RewritePanelProps {
  rewrites: RewriteCard[];
}

export function RewritePanel({ rewrites }: RewritePanelProps) {
  return (
    <section className="rounded-[1.6rem] border border-slate-200/80 bg-white p-5 shadow-[0_18px_36px_-34px_rgba(15,23,42,0.28)] transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_18px_36px_-34px_rgba(2,6,23,0.85)] sm:p-6">
      <div>
      ...
      <div className="mt-6 space-y-4">
        {rewrites.map(({ label, value, why, intent, target }) => (
          <div
            key={label}
            className="rounded-[1.4rem] border border-slate-200 bg-slate-50/90 p-4 transition hover:border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:hover:border-slate-600"
          >
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-3">
                ...
                <div className="h-2 overflow-hidden rounded-full bg-slate-200">
                  <div
                    className="h-full rounded-full bg-[linear-gradient(90deg,#0f172a_0%,#334155_75%,#64748b_100%)]"
                    style={{ width: `${Math.min(100, Math.max(18, Math.round((value.trim().length / (label === "Meta Description" ? 160 : label === "Title" ? 60 : 80)) * 100)))}%` }}
                  />
                </div>
