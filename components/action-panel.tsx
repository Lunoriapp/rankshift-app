...
import type { OptimisationStep } from "@/lib/audit-fixes";
...
export function ActionPanel({
  ...
  return (
    <section className="rounded-[1.9rem] border border-slate-200/80 bg-white p-6 shadow-[0_22px_50px_-36px_rgba(15,23,42,0.45)] transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_22px_50px_-36px_rgba(2,6,23,0.85)] sm:p-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="min-w-[15rem] rounded-[1.6rem] border border-slate-200 bg-slate-50/80 px-5 py-4 text-sm text-slate-700 transition-colors duration-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
          ...
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200">
            <div
              className="h-full rounded-full bg-[linear-gradient(90deg,#0f172a_0%,#334155_75%,#64748b_100%)] transition-all duration-500"
      ...
      <div className="mt-6 grid gap-3 lg:grid-cols-[1.7fr_0.9fr_0.9fr_0.9fr]">
        <button
          ...
          className="rounded-2xl bg-slate-950 px-5 py-4 text-sm font-semibold text-white shadow-[0_20px_45px_-20px_rgba(15,23,42,0.9)] transition hover:-translate-y-0.5 hover:bg-slate-800 dark:bg-sky-500 dark:text-slate-950 dark:hover:bg-sky-400"
        >
          {isPlanOpen ? "Hide Full Optimisation Plan" : "Generate Full Optimisation Plan"}
        </button>
        <button
          type="button"
          onClick={() => onLockedFeature("Save report")}
          className="rounded-2xl border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-400 dark:border-slate-700 dark:text-slate-300 dark:hover:border-slate-600"
        >
          Save Report
        </button>
        <button
          type="button"
          onClick={() => onLockedFeature("Export action plan")}
          className="rounded-2xl border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-400 dark:border-slate-700 dark:text-slate-300 dark:hover:border-slate-600"
        >
          Export Action Plan
        </button>
      ...
      {isPlanOpen ? (
        <div className="mt-8 space-y-4 rounded-[1.6rem] border border-slate-200 bg-slate-50/60 p-5 transition-colors duration-300 dark:border-slate-700 dark:bg-slate-800">
          ...
          {steps.map((step, index) => (
            <div
              key={step.id}
              className="rounded-[1.6rem] border border-slate-200 bg-white p-5 transition hover:border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:hover:border-slate-600"
            >
