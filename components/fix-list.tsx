...
import { MissingAltImageList } from "@/components/missing-alt-image-list";
...
interface FixListProps {
  ...
  onToggleFix: (fix: AuditFix, completed: boolean) => void;
  onToggleOpportunity: (
...
function formatPillarLabel(pillar: AuditFix["pillar"]): string {
...
function bySeverity(fixes: AuditFix[]) {
  return [
    {
      label: "Critical",
...
export function FixList({
  ...
  return (
    <section className="rounded-[1.75rem] border border-slate-200/80 bg-white p-6 shadow-[0_22px_45px_-36px_rgba(15,23,42,0.45)] transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_22px_45px_-36px_rgba(2,6,23,0.85)] sm:p-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="min-w-[14rem] rounded-[1.5rem] border border-slate-200 bg-slate-50/80 px-4 py-4 transition-colors duration-300 dark:border-slate-700 dark:bg-slate-800">
          ...
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
            <div
              className="h-full rounded-full bg-[linear-gradient(90deg,#047857_0%,#10b981_100%)] transition-all duration-500"
      ...
      {feedbackMessage ? (
        <div className="mt-6 rounded-[1.4rem] border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm leading-6 text-emerald-900 shadow-sm transition-all">
      ...
      <div className="mt-8 space-y-6">
        {grouped.map(({ label, items, accent }) => (
          <div key={label}>
            ...
            {items.length > 0 ? (
              <ul className="mt-4 space-y-4">
                {items.map((fix) => {
                  ...
                  return (
                    <li
                      key={fix.id}
                      className={`rounded-[1.55rem] border px-5 py-5 transition-all duration-300 ${accent} ${isCompleted ? "border-emerald-200 bg-emerald-50/80 shadow-[0_16px_35px_-26px_rgba(16,185,129,0.55)] dark:border-emerald-800 dark:bg-slate-800" : ""}`}
                    >
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div className="max-w-4xl space-y-4">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {isCompleted ? (
                              <span className="rounded-full border border-emerald-200 bg-emerald-100 px-2.5 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-emerald-800">
                                Completed
                              </span>
                            ) : null}
                          </div>
                          ...
                          {fix.id === "image-alt" ? (
                            <MissingAltImageList images={missingAltImages} />
                          ) : null}
                        </div>
                        <button
                          type="button"
                          onClick={() => onToggleFix(fix, !isCompleted)}
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm leading-7 text-slate-600 transition-colors duration-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
                {emptyState(label)}
              </div>
            )}
          </div>
        ...
        <InternalLinkingOpportunities
          opportunities={internalLinkOpportunities}
