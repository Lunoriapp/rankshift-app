...
function buildApproximateNeeds(data: CompetitorComparisonData): string[] {
  ...
  const averageWords = Math.round(
    data.competitors.reduce((sum, item) => sum + item.wordCount, 0) /
      data.competitors.length,
  );
  const averageLinks = Math.round(
      ...
      <div className="mt-6 rounded-[1.6rem] border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-950/70">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          {approximateNeeds.map((item) => (
            <span
              key={item}
              className="rounded-full bg-white px-3 py-2 text-sm font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-200"
            >
              {item}
            </span>
          ))}
        ...
        <section className="rounded-[1.6rem] bg-slate-50 px-5 py-5 dark:bg-slate-950/70">
          ...
          <ul className="mt-4 space-y-3">
            {insights.fixes.map((fix) => (
              <li key={fix} className="flex items-start gap-3 text-sm leading-6 text-slate-700 dark:text-slate-200">
