    ...
    <section className="space-y-4">
      {sections.map((section) => {
        ...
        return (
          <div
            key={section.id}
            className="overflow-hidden rounded-[1.6rem] border border-slate-200 bg-white transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900"
          >
            <button
              type="button"
              onClick={() => onToggleSection(section.id)}
              className="flex w-full items-center justify-between gap-4 px-5 py-5 text-left transition hover:bg-slate-100 dark:hover:bg-slate-800/70 sm:px-6"
            >
              <div>
