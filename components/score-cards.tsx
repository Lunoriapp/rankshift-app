import type { ScorePillar } from "@/lib/scorer";
...
export function ScoreCards({ items }: ScoreCardsProps) {
  return (
    <section className="space-y-4">
      ...
      {items.map((item) => (
        <div
          key={item.key}
          className="rounded-[1.75rem] border border-slate-200/80 bg-white p-5 shadow-[0_22px_45px_-38px_rgba(15,23,42,0.45)] transition hover:-translate-y-0.5 hover:border-slate-300 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700"
        >
          <div className="space-y-3">
            <p className="text-sm font-semibold leading-6 text-slate-500 dark:text-slate-300">{item.key}</p>
            <span
              className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] ${
                item.value.score === item.value.maxScore
          ...
          <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
            <div
              ...
              style={{ width: `${Math.round((item.value.score / item.value.maxScore) * 100)}%` }}
            />
          </div>
