    ...
    >
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-8">
        ...
        <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.35)] transition-colors duration-300 dark:border-slate-800 dark:bg-slate-950/90 dark:shadow-[0_24px_60px_-40px_rgba(2,6,23,0.85)] sm:p-8">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950 dark:text-slate-50">
                  See the exact page, fix, and next move
                </h2>
                <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600 dark:text-slate-300">
                  After you run an audit, Rankshift turns the page into a working SEO action plan
                  with scores, rewrite suggestions, internal links to add, and progress you can
                  revisit later.
                </p>
              </div>
              <Link
                href="/report/101"
                className="hidden rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-900 transition hover:border-slate-500 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-100 dark:hover:border-slate-500 dark:hover:bg-slate-900 sm:inline-flex"
              >
                View example report
              </Link>
            </div>

            <div className="mt-6 overflow-hidden rounded-[1.75rem] border border-slate-200 bg-[linear-gradient(180deg,#f8fafc_0%,#ffffff_100%)] shadow-[inset_0_1px_0_rgba(255,255,255,0.6)] transition-colors duration-300 dark:border-slate-800 dark:bg-[linear-gradient(180deg,#0f172a_0%,#020617_100%)]">
              <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 dark:border-slate-800">
                ...
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">
                  Audit workspace preview
                </p>
              </div>
              <div className="space-y-4 p-5">
                <div className="grid gap-4 lg:grid-cols-[0.92fr_1.08fr]">
                  <div className="rounded-[1.5rem] border border-red-200 bg-red-50/80 p-4 transition-colors duration-300 dark:border-red-900/80 dark:bg-red-950/50">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-red-700/80">
                      Priority page
                    </p>
                    <p className="mt-3 text-lg font-semibold tracking-tight text-slate-950 dark:text-slate-50">
                      Silver Splitter: Considering Divorce Later in Life
                    </p>
                    <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                      flip.co.uk/silver-splitter-considering-divorce
                    </p>
                    <div className="mt-4 flex items-end gap-3">
                      <span className="text-4xl font-semibold tracking-tight text-red-700">34</span>
                      <span className="pb-1 text-sm font-semibold text-red-700">Critical</span>
                    </div>
                    <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">
                      11 issues found across metadata, headings, images, and internal links
                    </p>
                  </div>

                  <div className="rounded-[1.4rem] border border-slate-200 bg-white p-4 transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/70 dark:text-emerald-300">
                        High confidence
                      </span>
                    </div>
                    <div className="mt-3 rounded-2xl bg-amber-50 px-3 py-3 text-sm font-medium text-slate-900 dark:bg-amber-950/60 dark:text-amber-100">
                      Suggested anchor: collaborative law
                    </div>
                    <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
                      Source snippet mentions the topic naturally, but no contextual body link is
                      supporting the related service page yet.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid gap-3 lg:grid-cols-3">
                  <div className="rounded-[1.4rem] border border-slate-200 bg-white p-4 transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-3">
                      {[
                        { label: "Previous", value: "46", tone: "text-slate-950" },
                        { label: "Current", value: "34", tone: "text-slate-950" },
                        { label: "Change", value: "-12", tone: "text-red-700" },
                      ].map((item) => (
                        <div key={item.label}>
                          <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">
                            {item.label}
                          </p>
                          <p className={`mt-2 text-xl font-semibold tracking-tight ${item.tone}`}>
                            {item.value}
                          </p>
                        </div>
                      ))}
                  ...
                  <div className="rounded-[1.4rem] border border-slate-200 bg-white p-4 transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                      Title, meta description, and H1 updates sit next to the issues so fixes are
                      ready to implement.
                    </p>
                  </div>

                  <div className="rounded-[1.4rem] border border-slate-200 bg-slate-50/80 p-4 transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900/80">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">
                      What you get
                    </p>
                    <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                      Saved reports, internal link opportunities, and scan history you can revisit
                      after the next run.
                    </p>
                  </div>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.35)] transition-colors duration-300 dark:border-slate-800 dark:bg-slate-950/90 dark:shadow-[0_24px_60px_-40px_rgba(2,6,23,0.85)] sm:p-8">
              ...
              <p className="mt-3 text-sm leading-7 text-slate-600 dark:text-slate-300">
                Open a sample report to see how Rankshift turns one page audit into a score,
                priority fixes, internal links, rewrites, and scan history.
              </p>
              <Link
                href="/report/101"
                className="mt-5 inline-flex rounded-2xl border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-900 transition hover:border-slate-500 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-100 dark:hover:border-slate-500 dark:hover:bg-slate-900"
              >
                View example report
              </Link>
            </div>
