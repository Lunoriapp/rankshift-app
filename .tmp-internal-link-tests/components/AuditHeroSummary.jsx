...
exports.AuditHeroSummary = AuditHeroSummary;
function formatCompetitorGap(value) {
    if (value <= 0) {
        return "Level";
    }
    return `-${value} pts`;
}
function AuditHeroSummary({ url, score, issuesFound, competitorGap, status, headline, summary, onPrimaryAction, onSecondaryAction, }) {
    const statusTone = status === "Strong"
    ...
    return (<section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_30px_80px_-40px_rgba(15,23,42,0.3)] transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_30px_80px_-40px_rgba(2,6,23,0.8)]">
      <div className="grid gap-8 p-6 sm:p-8 xl:grid-cols-[1.2fr_0.8fr] xl:p-10">
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button type="button" onClick={onPrimaryAction} className="rounded-2xl bg-sky-500 px-5 py-3.5 text-sm font-semibold text-slate-950 transition hover:bg-sky-400">
              Fix this page now
            </button>
            <button type="button" onClick={onSecondaryAction} className="rounded-2xl border border-slate-300 px-5 py-3.5 text-sm font-semibold text-slate-700 transition hover:border-slate-400 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:border-slate-600 dark:hover:bg-slate-800">
              View top fixes
            </button>
          </div>
