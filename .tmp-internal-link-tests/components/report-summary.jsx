...
function ReportSummary({ audit, opportunity, completedFixCount, totalFixCount, }) {
    const progressPercent = totalFixCount > 0 ? Math.round((completedFixCount / totalFixCount) * 100) : 0;
    const remainingFixes = Math.max(totalFixCount - completedFixCount, 0);
    return (<section className="overflow-hidden rounded-[2.1rem] border border-slate-200/80 bg-white shadow-[0_30px_80px_-40px_rgba(15,23,42,0.35)] transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_30px_80px_-40px_rgba(2,6,23,0.85)]">
        ...
        <div className="space-y-7">
          ...
          <div className="rounded-[1.7rem] border border-slate-200 bg-slate-50/80 p-4 transition-colors duration-300 dark:border-slate-700 dark:bg-slate-800">
            ...
            <div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-200">
              <div className="h-full rounded-full bg-[linear-gradient(90deg,#0f172a_0%,#334155_68%,#64748b_100%)] transition-all duration-500" style={{ width: `${progressPercent}%` }}/>
            </div>
          </div>
        ...
        <div className="relative flex flex-col justify-between gap-6 overflow-hidden rounded-[1.9rem] bg-slate-950 p-6 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] sm:p-8">
          ...
          <div className="relative space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <link_1.default href="/" className="rounded-2xl border border-white/15 bg-white/5 px-4 py-3 text-center text-sm font-semibold text-white transition hover:bg-white/10">
                Re-run Audit
              </link_1.default>
              <link_1.default href="/reports" className="rounded-2xl border border-white/15 bg-transparent px-4 py-3 text-center text-sm font-semibold text-slate-300 transition hover:bg-white/5">
                View Saved Reports
              </link_1.default>
            </div>
