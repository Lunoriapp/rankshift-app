...
exports.WorkspacePageRow = WorkspacePageRow;
...
function WorkspacePageRow({ page }) {
    const delta = (0, workspace_page_list_1.getScoreDelta)(page.score, page.previousScore);
    const tones = priorityStyles(page.priority);
    return (<link_1.default href={`/report/${page.id}`} className={`group block rounded-[28px] border px-5 py-5 shadow-[0_14px_40px_rgba(15,23,42,0.06)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_18px_45px_rgba(15,23,42,0.1)] ${tones.row}`}>
      <div className="flex flex-col gap-5 lg:grid lg:grid-cols-[minmax(0,1.8fr)_minmax(0,1fr)] lg:items-center">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className={`rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${tones.badge}`}>
        ...
        <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-[0.85fr_0.9fr_0.9fr] lg:items-center">
          <div>
            ...
            <div className="mt-2 flex items-end gap-3">
              ...
              <span className={`pb-1 text-sm font-semibold ${deltaTone(delta)}`}>
                {(0, workspace_page_list_1.formatScoreDelta)(delta)}
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="mt-2 text-sm font-medium text-slate-900">
              {(0, workspace_page_list_1.formatLastScannedDate)(page.lastScanned)}
            </p>
          </div>
